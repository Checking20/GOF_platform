...
def AddMap(request):
    if request.method == "POST":
        content = request.POST.get("mapContent")
        #username = request.POST.get("username")
        username = request.COOKIES["username"]
        #return HttpResponse(json.dumps({'data': {'username': username}}))
        try:
    ...
    if request.method == "GET":
        ...
        if type == "hot":
            try:
                ...
                mapp = getMap(state_list.Map_ID)
                return HttpResponse(
                    json.dumps({'data': {"flag": True, "state_list": state_list, "map_content": mapp.Content}}))
            except:
                return HttpResponse(json.dumps({'data': {'flag': False}}))
        #最近热门（待完成）
        if type == "recent_hot":
